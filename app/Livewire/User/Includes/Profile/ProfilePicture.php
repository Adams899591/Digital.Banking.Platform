...
class ProfilePicture extends Component
{
    ...
    public function updateProfilePhoto(){
        ...
        if ($path) {    
            ...
            $mediaUrl = Storage::disk('s3')->url($path); // this hold  the insert image url

            // only try to delete old image if the image id is not null
            if ($user->profile_picture_id != null) {
                Storage::disk('s3')->delete($user->profile_picture_id);
            }
         ...
         $user->profile_picture = $mediaUrl;  // insert the image url 
         $user->profile_picture_id = $path;   // indert the image id 
         $user->save();
